"""
Graph Attention Network (GAT) encoder implementation.

This module contains the implementation of a GAT-based encoder for converting
...
from causal_meta.inference.models.graph_utils import normalize_adjacency, preprocess_node_features
...
class GATEncoder(BaseGNNEncoder):
    ...
    This encoder uses multiple GAT layers with multi-head attention to process node features,
    ...
    def __init__(
        ...
        **kwargs
        ...
        Args:
            ...
            pooling: Pooling method for graph-level embeddings ("mean", "max", "sum", "attention")
            batch_norm: Whether to use batch normalization
            heads: Number of attention heads
            concat_heads: Whether to concatenate or average the attention heads
        ...
        # First layer: input_dim -> hidden_dim (with multi-head attention)
        if concat_heads:
            # Ensures concatenated output is hidden_dim
            first_layer_out_dim = hidden_dim // heads
        else:
            first_layer_out_dim = hidden_dim  # Output dim is preserved with averaged heads

        self.convs.append(GATConv(input_dim, first_layer_out_dim,
        ...
        for i in range(num_layers - 2):
            if concat_heads:
                middle_layer_out_dim = hidden_dim // heads
            else:
                middle_layer_out_dim = hidden_dim

            self.convs.append(GATConv(hidden_dim, middle_layer_out_dim,
                              heads=heads, concat=concat_heads, dropout=dropout))
        ...
        if num_layers > 1:
            if concat_heads:
                last_layer_out_dim = latent_dim // heads
            else:
                last_layer_out_dim = latent_dim

            self.convs.append(GATConv(hidden_dim, last_layer_out_dim,
                              heads=heads, concat=concat_heads, dropout=dropout))
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, GATConv):
            # Initialize lin, att weights with xavier_uniform_
