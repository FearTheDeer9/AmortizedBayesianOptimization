...
class ABCDStrategy(AcquisitionStrategy):
    def _score_for_value(self, nodes, value_samplers, current_logpdfs=None, datapoints=None):
        ...
        if datapoints is None:
            datapoints = self.model.sample_interventions(nodes, value_samplers, self.num_samples)
        ...
        _extras = {"intervention_logpdfs": intervention_logpdfs}

        return posterior_entropy, _extras


class GreedyABCDStrategy(ABCDStrategy):
    def acquire(self, nodes, iteration):
        ...
        for _ in range(self.args.batch_size):
            ...
            interventional_logpdf = strategy.extra[strategy.max_iter][
                "intervention_logpdfs"
            ]
