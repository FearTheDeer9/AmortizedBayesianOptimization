...
from utils import all_combinations, expm_np
...
class GraphDistribution:
    ...
    def log_normalization_constant(self, all_g):
        ...
        for i, g in enumerate(
            tqdm.tqdm(
                all_g, desc="p(G) log_normalization_constant", disable=not self.verbose
            )
        ):
...
class UniformDAGDistributionRejection(GraphDistribution):
    ...
    def sample_G(self, return_mat=False):
        ...
        while True:
            mat = np.random.choice(2, size=self.n_vars * self.n_vars).reshape(
...
class GibbsUniformDAGDistribution(GraphDistribution):
    ...
    def __init__(self, n_vars, gibbs_temp=10.0, sparsity_factor=0.0, verbose=False):
        super(GibbsUniformDAGDistribution, self).__init__(
            n_vars=n_vars, verbose=verbose
        )
        self.n_vars = n_vars
        self.verbose = verbose
...
class GibbsDAGDistributionFull(GraphDistribution):
    ...
    def __init__(self, n_vars, gibbs_temp=10.0, sparsity_factor=0.0, verbose=False):
        ...
        assert (
            n_vars <= 4
        ), "Cannot use this for higher dimensional variables, Try UniformDAGDistributionRejection instead"
        self.n_vars = n_vars
        self.verbose = verbose
        self.gibbs_temp = gibbs_temp
        self.sparsity_factor = sparsity_factor
        all_g = all_combinations(
            n_vars, return_adj=True
        )  # Do not store this in interest of memory
        dagness = np.zeros(len(all_g))
        for i, j in enumerate(all_g):
            dagness[i] = expm_np(j, self.n_vars)
        self.logits = -gibbs_temp * dagness - sparsity_factor * np.sum(
            all_g, axis=(-1, -2)
        )
    ...
    def sample_G(self, return_mat=False):
        ...
        mat_id = torch.distributions.Categorical(
