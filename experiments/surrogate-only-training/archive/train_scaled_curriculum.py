...
from src.causal_bayes_opt.utils.checkpoint_utils import save_checkpoint, load_checkpoint
...
class ScaledCheckpointManager:
    ...
    def save_stage_checkpoint(self, params, opt_state, stage_info: Dict):
        ...
        save_checkpoint(
            ...
            params=params,
            architecture={
                'hidden_dim': 128,  # Default from curriculum config
                'num_layers': 8,
                'num_heads': 8,
                'key_size': 16,
                'dropout': 0.1,
    ...
    def save_best_for_size(self, params, metrics: Dict, num_vars: int):
        ...
        if path.exists():
            ...
            existing_f1 = existing.get('metrics', {}).get('f1', 0)
            if metrics.get('f1', 0) <= existing_f1:
                return False
