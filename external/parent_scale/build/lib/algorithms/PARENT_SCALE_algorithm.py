...
class PARENT_SCALE(BASE):
    ...
    def determine_initial_probabilities(self) -> Dict[Tuple, float]:
        if self.use_doubly_robust:
            ...
            # CRITICAL FIX: Use reasonable parent set combinations instead of exponential explosion
            # Original code generated up to 10+ variable combinations, causing array mismatches
            combinations = []
            # Only consider individual variables and pairs (reasonable for most causal discovery)
            max_parents = min(3, len(variables))  # Limit to reasonable parent set sizes
            for r in range(1, max_parents + 1):
                combinations.extend(itertools.combinations(variables, r))
            
            # Add empty set as possibility (no parents)
            combinations.append(())
