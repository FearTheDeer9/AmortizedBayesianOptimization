...
def main(cfg: DictConfig) -> None:
    """Run the complete scaling experiment with Hydra configuration and WandB tracking."""
    logger.info("Starting Erdos-Renyi scaling experiment with Hydra + WandB")
    ...
    if cfg.get("wandb", {}).get("enabled", False):
        wandb.init(
            ...
            name=f"{cfg.experiment.name}_{cfg.min_nodes}-{cfg.max_nodes}nodes"
        )
    ...
    config = ScalingExperimentConfig(
        ...
        n_intervention_steps=cfg.n_intervention_steps,
        n_observational_samples=cfg.n_observational_samples,
        learning_rate=cfg.learning_rate,
        n_runs_per_config=cfg.n_runs_per_config,
        base_random_seed=cfg.base_random_seed
    )
    
    # Log experiment start
    if wandb.run:
        wandb.log({"experiment_status": "started", "config": asdict(config)})
    ...
    if wandb.run:
        ...
        wandb.log({
            ...
            "learning_win_rate": overall['learning_win_rate'],
            "mean_f1_improvement": overall['mean_f1_improvement'],
            "learning_wins_count": overall['learning_wins_count'],
            "total_graph_sizes": overall['total_graph_sizes'],
            "experiment_status": "completed"
        })
        
        # Log per-graph-size results
        for size_result in analysis['per_size_analysis']:
            wandb.log({
                f"graph_size_{size_result['graph_size']}_learning_wins": size_result['learning_wins'],
                f"graph_size_{size_result['graph_size']}_f1_improvement": size_result['mean_f1_improvement'],
                f"graph_size_{size_result['graph_size']}_learning_f1": size_result['mean_learning_f1'],
                f"graph_size_{size_result['graph_size']}_baseline_f1": size_result['mean_baseline_f1']
            })
        ...
        for result in results:
            summary_data.append({
                ...
                "learning_f1": result.learning_f1,
                "baseline_f1": result.baseline_f1,
                "improvement": result.learning_f1 - result.baseline_f1,
                "learning_wins": result.learning_f1 > result.baseline_f1
            })
        
        summary_df = pd.DataFrame(summary_data)
        wandb.log({"results_summary": wandb.Table(dataframe=summary_df)})
    
    # Save results (existing functionality)
    ...
    success = overall['learning_win_rate'] > 0.6
    if success:
        print("✅ Validation SUCCESSFUL - Learning approach works!")
        if wandb.run:
            wandb.log({"validation_result": "success"})
    else:
        print("❌ Validation INCONCLUSIVE - Need to investigate further")
        if wandb.run:
            wandb.log({"validation_result": "inconclusive"})
