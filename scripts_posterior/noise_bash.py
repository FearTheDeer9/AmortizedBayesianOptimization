...
run_nums = range(1, 11)
graphs = ["Erdos5", "Erdos10", "Erdos15", "Erdos20"]
n_obs_samples = [200]
sigma_list = [0.1, 1, 2, 3]
        ...
        for sigma in sigma_list:
            for n_obs in n_obs_samples:
                ...
                command = (
                    ...
                    f"--run_num {run_num} --p 1 --noise {sigma} "
                    f'--graph_type "{graph}" --use_dr  --include_nint '
                )
                os.system(command)
                ...
                command = (
                    f"python3 erdos_renyi_posterior.py --seeds_replicate {seed} --n_obs {n_obs} "
                    f"--run_num {run_num} --nonlinear --p 1 --noise {sigma} "
                    f'--graph_type "{graph}" --use_dr  --include_nint '
                )
                os.system(command)
                print(f"Executed: {command}")
